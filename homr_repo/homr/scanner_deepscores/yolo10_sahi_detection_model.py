...
class Yolov10DetectionModel(DetectionModel):
    ...
    def load_model(self):
        ...
        """
        # Try different import methods for YOLOv10 compatibility
        model_class = None
        try:
            # First try the newer ultralytics with YOLOv10 support
        ...
        if model_class is None:
            try:
                # Fall back to YOLO class which can load YOLOv10 models
                from ultralytics import YOLO
                model_class = YOLO
            except ImportError:
                raise ImportError("Could not import YOLO from ultralytics")

        try:
            # Try to add safe globals for PyTorch 2.6+ compatibility
            try:
                from ultralytics.nn.tasks import YOLOv10DetectionModel as UltralyticsYOLOv10DetectionModel
                torch.serialization.add_safe_globals([UltralyticsYOLOv10DetectionModel])
            except (ImportError, AttributeError):
                pass  # Not needed for all versions

            model = model_class(self.model_path)
